...
let veryOldInventors = inventors.filter((inventor, index, array) => {
...
let inventorNames = inventors.map((inventor, index, array) => {
...
let inventorNamedAda = inventors.find((inventor, index, array) => {
...
let firstLast = people.map((name, index, array) => {
  rename = name.split(", ")
  firstName = rename[1]
  lastName = rename[0]
...
let isAdultPresent = devs.some((dev, index) => {
...
travelMethodCounts = travelMethods.reduce((acc, type) => {
  if (acc[type]) {
    acc[type]++
  } else {
    acc[type] = 1
